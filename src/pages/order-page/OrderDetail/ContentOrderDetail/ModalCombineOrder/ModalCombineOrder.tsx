...
const ModalCombineOrder = (props: Props) => {
  ...
  const handleChangeValue = (id_table: any) => {
    setLoading(true);
    invoiceServices
      .getInvoiceByIdTable(id_table)
      .then((res: any) => {
        if (res.status) {
          const name_order = Array.isArray(res?.data?.tablefood_invoices)
            ? res?.data?.tablefood_invoices
                .map((item: any) => item?.id_table)
                .join(",")
            : "";
          form.setFieldValue("id_invoice_new", `Yêu cầu bàn ${name_order}`);
          const array_detail_yeu_cau_ghep = Array.isArray(
            res?.data.invoice_details
          )
            ? res?.data.invoice_details.map((item: any) => {
                return {
                  id: item?.id,
          ...
          addToMap(dataSource);
          const array = Array.from(invoice_details.values());
          const dataSet = array.map((item) => {
            return {
              name: item?.name || "",
            ...
            <Col span={13}>
              ...
              >
                <Select
                  loading={loading}
                  allowClear
                  placeholder="Chọn yêu cầu muốn ghép"
                />
              </FormItem>
            </Col>
            <Col
            ...
            <Col span={16}>
              <Form.Item>
                ...
                >
                  ...
                  <Button
                    style={{ width: "80px", marginLeft: "7px" }}
                    onClick={() => {
                      handleModal();
                      form.resetFields();
                    }}
