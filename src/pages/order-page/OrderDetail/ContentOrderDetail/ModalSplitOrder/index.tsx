    ...
    try {
      ...
      curData.forEach((item: any) => {
        if (!inputValues[item.id] || inputValues[item.id] === "") {
          ...
          const check_amount_0 =
            parseInt(item.amount) - parseInt(inputValues[item.id]);
          if (check_amount_0 > 0) {
            old_invoice_detail.push({
              id_invoice: id_invoice_old,
              id_product: !item?.isCombo ? item?.id_product : null,
      ...
      const res_create_new_order = await invoiceServices.create(
        dataCreateNewOrder
      );
      const res_update_old_order = await invoiceServices.update(
        id_invoice_old,
        dataUpdateOldOrder
      );
      ...
      if (res_update_old_order.status && res_create_new_order) {
        ...
        invoiceServices
          .getById(res_update_old_order.data.id)
          .then((res: any) => {
            if (res.status) {
